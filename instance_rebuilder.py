...
def rebuild_instance(api_key, instance_id):
    ...
    onstart_script = f"wget https://github.com/woodysoil/XenblocksMiner/releases/download/v1.1/xenblocksMiner-v1.1.1-Linux-x86_64.tar.gz && tar -vxzf xenblocksMiner-v1.1.1-Linux-x86_64.tar.gz && chmod +x xenblocksMiner && (echo -e \"{eip55_address}\\n0\" | sudo nohup ./xenblocksMiner >> miner.log 2>&1 &) && (while true; do sleep 10; : > miner.log; done) &"
    payload = {
...
def update_onstart_script(ssh_host, ssh_port, username, private_key_path, instance_id, passphrase=None):
    ...
    try:
        ...
        update_command = f'echo "wget https://github.com/woodysoil/XenblocksMiner/releases/download/v1.1/xenblocksMiner-v1.1.1-Linux-x86_64.tar.gz && tar -vxzf xenblocksMiner-v1.1.1-Linux-x86_64.tar.gz && chmod +x xenblocksMiner && (echo -e \\"{eip55_address}\\\\n0\\" | sudo nohup ./xenblocksMiner >> miner.log 2>&1 &) && (while true; do sleep 10; : > miner.log; done) &" > /root/onstart.sh'
